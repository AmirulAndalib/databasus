...
export const ru: typeof en = {
  ...
  errors: {
    ...
    emailNotConfigured: 'Почтовый сервер не настроен.',
    adminEmailMissing:
      'У вашего аккаунта нет корректного адреса почты, на который можно отправить тестовое письмо.',
  },
  status: {
    logicalBackup: {
  ...
  settings: {
    ...
    mailServer: {
      ...
      testEmailSent: 'Тестовое письмо отправлено на {{email}}',
    },
    healthcheck: {
  ...
  notifiers: {
    ...
    email: {
      ...
      from: 'Отправитель',
      fromTooltip:
        'Необязательно. Адрес отправителя, можно с именем: Acme Backups <noreply@example.com>. Если поле пустое, используется пользователь SMTP, когда это адрес почты, а иначе noreply@ и хост SMTP',
      ...
      securityTooltip:
        'TLS шифрует соединение с самого начала, обычно на порту 465. STARTTLS переводит соединение в зашифрованное и не отправляет письмо, если сервер этого не поддерживает. «Без шифрования» передает пароль и письма открытым текстом — выбирайте этот режим только для доверенного релея без STARTTLS.',
      securityModes: {
        tls: 'TLS',
        starttls: 'STARTTLS',
        none: 'Без шифрования',
      },
      heloName: 'Имя в приветствии',
      heloNamePlaceholder: 'Автоматически',
      heloNameTooltip:
        'Имя, которым Databasus представляется почтовому серверу (EHLO). Если поле пустое, берется хост из DATABASUS_URL или имя машины.',
    },
    telegram: {
