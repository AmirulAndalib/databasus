        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              ...
              {
                "@type": "Question",
                name: "Я забыл email или пароль администратора",
                acceptedAnswer: {
                  "@type": "Answer",
                  text: 'Первая учетная запись, созданная на инстансе, им и управляет. Если вы не помните, какой это адрес, выполните docker exec -it databasus ./main --list-admins на сервере, где запущен Databasus — команда перечислит все учетные записи администраторов с их email, именем, датой создания и состоянием активности и не выведет ни одного пароля. Чтобы задать новый пароль этой учетной записи, выполните docker exec -it databasus ./main --new-password="YourNewSecurePassword123" --email="owner@example.com". Инстанс, созданный до того, как первая учетная запись стала им управлять, до сих пор носит адрес-заглушку admin, пока владелец его не заменит. Обе команды описаны на странице про пароль.',
                },
              },
            ],
          }),
      ...
      <section id="faq" className="py-12 md:py-20 px-4 md:px-6 lg:px-0">
        ...
        <div className="mx-auto w-full max-w-[1000px] 2xl:max-w-[1200px]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
            ...
            <FaqItem
              number="15"
              question="Я забыл email или пароль администратора"
              answer={
                <>
                  Первая учетная запись, созданная на инстансе, им и управляет.
                  Если вы не помните, какой это адрес, выполните docker exec -it
                  databasus ./main --list-admins на сервере, где запущен
                  Databasus — команда перечислит все учетные записи
                  администраторов с их email, именем, датой создания и
                  состоянием активности и не выведет ни одного пароля. Чтобы
                  задать новый пароль этой учетной записи, выполните docker exec
                  -it databasus ./main
                  --new-password=&quot;YourNewSecurePassword123&quot;
                  --email=&quot;owner@example.com&quot;. Инстанс, созданный до
                  того, как первая учетная запись стала им управлять, до сих пор
                  носит адрес-заглушку admin, пока владелец его не заменит. Обе
                  команды описаны на
                  <a
                    href="/ru/password"
                    className="text-blue-400 hover:text-blue-600"
